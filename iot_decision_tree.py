...
# Add argument
parser.add_argument('-z', required=False, help='path to containing zipfile', default=None)
...
if zipFileName is not None:
    with ZipFile(zipFileName, 'r') as zip:
        try:
            with zip.open(input, 'r') as file:
                Set1 = pd.read_csv(file)
...
X = [[j if type(j) != type('') else int(j, 16) for j in i[0:10]] for i in Set]
Y = [i[11] for i in Set]
...
if zipFileName is not None:
    with ZipFile(zipFileName, 'r') as zip:
        try:
            with zip.open(test, 'r') as file:
                Set2 = pd.read_csv(file)
        except Exception as e:
            print(e)
            zip.printdir()
else:
...
Xt = [[j if type(j) != type('') else int(j, 16) for j in i[0:10]] for i in Set_t]
